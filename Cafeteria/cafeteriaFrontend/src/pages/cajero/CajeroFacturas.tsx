...
import { FileText, Eye, Search, Download, Mail } from 'lucide-react';
...
export default function CajeroFacturas() {
  ...
  const filtered=search?facturas.filter((f:any)=>f.numeroFactura?.toLowerCase().includes(search.toLowerCase())||(f.cliente&&`${f.cliente.nombre} ${f.cliente.apellido}`.toLowerCase().includes(search.toLowerCase()))):facturas;
  ...
  const descargarPdf=async(id:number,num:string)=>{
    try{
      ...
      window.URL.revokeObjectURL(url);toast.success('PDF descargado');
  ...
  const enviarEmail=async(id:number)=>{
    ...
    if(!email)return;
    try{await facturaService.enviarEmail(id,email);toast.success('Factura enviada por email');}catch{toast.error('Error al enviar email');}
  };

  if(loading) return <LoadingSpinner/>;
  return(
    <div className="space-y-6">
      <div><h1 className="text-2xl font-display font-bold text-gray-900 dark:text-white">Facturas</h1></div>
      <div className="relative max-w-md"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400"/><input className="input pl-9" placeholder="Buscar por número o cliente..." value={search} onChange={e=>setSearch(e.target.value)}/></div>
      {filtered.length===0?<EmptyState title="Sin facturas" icon={FileText}/>:(
        <div className="space-y-3">{filtered.map((f:any)=>(
          <div key={f.id} className="card p-4 hover:shadow-sm transition-shadow">
            <div className="flex items-center justify-between flex-wrap gap-2">
              ...
              <div className="flex gap-1">
                <button onClick={()=>setDetail(f)} className="btn-ghost btn-xs" title="Ver detalle"><Eye className="w-3.5 h-3.5"/></button>
                <button onClick={()=>descargarPdf(f.id,f.numeroFactura)} className="btn-ghost btn-xs" title="Descargar PDF"><Download className="w-3.5 h-3.5"/></button>
                <button onClick={()=>enviarEmail(f.id)} className="btn-ghost btn-xs" title="Enviar por email"><Mail className="w-3.5 h-3.5"/></button>
              </div>
            </div>
          </div>
        ))}</div>
      ...
      <Modal open={!!detail} onClose={()=>setDetail(null)} title={`Factura ${detail?.numeroFactura||''}`}>
        {detail&&<div className="space-y-4">
          <div className="grid grid-cols-2 gap-3 text-sm">
            ...
            <div><span className="text-gray-500">Email:</span><p className="font-medium text-gray-900 dark:text-white">{detail.cliente?.email||'—'}</p></div>
          ...
          <div className="flex gap-2">
            <button onClick={()=>descargarPdf(detail.id,detail.numeroFactura)} className="btn-gold flex-1"><Download className="w-4 h-4"/>Descargar PDF</button>
            <button onClick={()=>enviarEmail(detail.id)} className="btn-outline flex-1"><Mail className="w-4 h-4"/>Enviar Email</button>
          </div>
        </div>}
      </Modal>
    </div>
