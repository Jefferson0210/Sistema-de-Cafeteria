...
export default function MeseroNuevoPedido() {
  ...
  useEffect(()=>{
    Promise.all([productService.disponibles(),mesaService.disponibles()])
    .then(([p,m])=>{setProductos(p.data.data||[]);setMesas(m.data.data||[]);})
    .catch(()=>{}).finally(()=>setLoading(false));
  },[]);

  const addItem=(p:Producto)=>{
    toast.success(`${p.nombre} agregado`,{icon:'🛒',duration:1500});
  ...
  const enviar=async()=>{
    ...
    setSending(true);
    try{
      await pedidoService.crear({mesaId,meseroId:user!.id,notas,items:items.map(i=>({productoId:i.producto.id,cantidad:i.cantidad,notas:i.notas}))});
  ...
  if(success) return(
    <div className="flex items-center justify-center py-20">
      <div className="text-center"><CheckCircle className="w-16 h-16 text-green-500 mx-auto mb-4"/><h2 className="text-2xl font-display font-bold text-gray-900 dark:text-white mb-2">¡Pedido Creado!</h2><p className="text-gray-500 mb-6">El pedido fue enviado a cocina</p>
      <div className="flex gap-3 justify-center"><button onClick={()=>{setSuccess(false);setItems([]);setMesaId(null);setNotas('');}} className="btn-gold">Nuevo Pedido</button><button onClick={()=>navigate('/mesero/pedidos')} className="btn-outline">Ver Pedidos</button></div></div>
    </div>
  );
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-4">
          <div className="card p-4">
            ...
            <select className="input" value={mesaId||''} onChange={e=>setMesaId(+e.target.value||null)}>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">{filteredProds.slice(0,18).map(p=>(
            <button key={p.id} onClick={()=>addItem(p)} className="card p-3 text-left hover:shadow-md hover:border-gold-300 transition-all group">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-brand-50 dark:bg-brand-900/20 rounded-lg flex items-center justify-center text-lg flex-shrink-0 group-hover:scale-110 transition-transform">🍽️</div>
                <div className="min-w-0 flex-1"><p className="text-sm font-medium text-gray-900 dark:text-white truncate">{p.nombre}</p><p className="text-brand-500 font-bold text-sm">{money(p.precio)}</p></div>
                <Plus className="w-4 h-4 text-gray-300 group-hover:text-gold-500"/>
              </div>
            </button>
          ))}</div>
        </div>
        <div className="card p-5 sticky top-24 h-fit">
          <h3 className="font-semibold text-gray-900 dark:text-white mb-4">Resumen del Pedido</h3>
          {items.length===0?<p className="text-gray-400 text-sm py-8 text-center">Agrega productos al pedido</p>:(
            <div className="space-y-3 mb-4">{items.map(i=>(
              <div key={i.producto.id} className="flex items-center gap-2">
                <div className="flex-1 min-w-0"><p className="text-sm font-medium text-gray-900 dark:text-white truncate">{i.producto.nombre}</p><p className="text-xs text-gray-400">{money(i.producto.precio)} c/u</p></div>
                <div className="flex items-center gap-1">
                  <button onClick={()=>updateQty(i.producto.id,i.cantidad-1)} className="w-6 h-6 border dark:border-gray-700 rounded flex items-center justify-center hover:bg-gray-50 dark:hover:bg-gray-800"><Minus className="w-3 h-3"/></button>
                  <span className="w-6 text-center text-sm font-bold">{i.cantidad}</span>
                  <button onClick={()=>updateQty(i.producto.id,i.cantidad+1)} className="w-6 h-6 border dark:border-gray-700 rounded flex items-center justify-center hover:bg-gray-50 dark:hover:bg-gray-800"><Plus className="w-3 h-3"/></button>
                </div>
          ...
          <div className="border-t dark:border-gray-800 pt-3 space-y-1.5 mb-4">
